...
const AffiliateGroupsDetailContainer: React.FC = () => {
  ...
  const { data: groupData, loading, error } = useSelector(
    (state: RootState) => state.groupDetail || { data: null, loading: false, error: null }
  );
  ...
  const activeMethods = groupData.paymentMethods?.filter((m) => m.isActive) || [];
    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 px-6 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="text-right">
          ...
          <p className="text-lg font-semibold text-slate-900">{holder?.firstName} {holder?.lastName}</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-blue-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-2">{groupData.plan?.name || 'N/A'}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-emerald-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-2">{groupData.affiliates?.length || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-amber-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-2">{activeMethods.length}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-red-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-2">${billingBalance.toFixed(2)}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900">Información del Grupo</h2>
              <button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs uppercase text-slate-500 font-semibold">Nombre del Grupo</p>
                  <p className="text-lg font-semibold text-slate-900 mt-1">{groupData.name}</p>
                </div>
                <div>
                  <p className="text-xs uppercase text-slate-500 font-semibold">Titular</p>
                  <p className="text-lg font-semibold text-slate-900 mt-1">{groupData.holderFullName}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs uppercase text-slate-500 font-semibold">DNI Titular</p>
                  <p className="text-lg font-mono text-slate-900 mt-1">{holder?.documentNumber}</p>
                </div>
                <div>
                  <p className="text-xs uppercase text-slate-500 font-semibold">Estado</p>
                  <span className={`inline-block mt-1 px-3 py-1 rounded-lg text-sm font-semibold ${
                    groupData.isActive
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs uppercase text-slate-500 font-semibold">Fecha de Inscripción</p>
                  <p className="text-lg font-semibold text-slate-900 mt-1">
                    {new Date(groupData.createdAt).toLocaleDateString('es-AR')}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase text-slate-500 font-semibold">Calificación</p>
                  <p className="text-lg font-semibold text-slate-900 mt-1">{groupData.rating}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900">Miembros del Grupo</h2>
              <button
                ...
                <tbody className="divide-y divide-slate-100">
                  {groupData.affiliates?.map((affiliate) => (
                    <tr key={affiliate.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 font-mono text-slate-600 text-xs">{affiliate.documentNumber}</td>
                      <td className="px-4 py-3 text-slate-600">
                        {new Date(affiliate.birthDate).toLocaleDateString('es-AR')}
                      </td>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900">Formas de Pago</h2>
              <button
            ...
            {activeMethods.length > 0 ? (
              ...
              <p className="text-slate-500 text-center py-8">No hay formas de pago registradas</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              <div>
                <p className="text-xs uppercase text-slate-500 font-semibold">Nombre</p>
                <p className="text-lg font-semibold text-slate-900 mt-1">{groupData.plan?.name || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs uppercase text-slate-500 font-semibold">Cuota Mensual</p>
                <p className="text-lg font-semibold text-slate-900 mt-1">
                
                </p>
              </div>
              <div>
                <p className="text-xs uppercase text-slate-500 font-semibold">Estado del Plan</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-900">Pagos Recientes</h2>
              <button
            ...
            {recentPayments.length > 0 ? (
              ...
              <p className="text-slate-500 text-center py-6 text-sm">Sin pagos registrados</p>
            )}
          </div>

          {/* Status Overview */}
          <div className="bg-white rounded-xl shadow-sm p-6">
            <h2 className="text-lg font-bold text-slate-900 mb-4">Estado General</h2>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600">Período de Gracia</span>
